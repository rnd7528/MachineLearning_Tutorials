...
A = tf.constant(np.reshape(np.arange(2.0, 8.0, dtype=np.float32), (2, 3)))
print(A.get_shape())
print(A.dtype)
print(A.eval())
...
#       Python API. You’ll find similarly named functions.
...
# if x < y, execute f1, else execute f2
A = tf.cond(tf.greater(x, y), f1, f2)
A.eval()